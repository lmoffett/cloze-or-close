...
# Generates the list of hot replacement candidates
...
candidates = pd.read_csv(prep_path/'hot'/'hot_perturbation_metadata.csv')
...
replacement_series = candidates['replacement_candidates'].progress_apply(perturb_all)
logger.info("candidates perturbed")

results_path = Path(os.environ['CORC_DATASETS_PREP_DIR']) / 'hot'
csv_path = (results_path / f'hot-{args.dataset_name}').with_suffix(".csv")
